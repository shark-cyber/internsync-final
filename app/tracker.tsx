import React, { useState, useEffect } from "react";
import {
  View,
  Text,
  StyleSheet,
  Pressable,
  Image,
  ScrollView,
  Modal,
  ActivityIndicator,
  Alert,
} from "react-native";
import { useRouter } from "expo-router";
import { useAuth } from "../src/contexts/AuthContext";
import { Portal } from "../src/components/Portal";
import { LinearGradient } from "expo-linear-gradient";
import { useSafeAreaInsets } from "react-native-safe-area-context";
import { Ionicons } from "@expo/vector-icons";
import { colors, font, radius } from "../src/theme";
import { Menu } from "../src/components/Menu";
import { Glow } from "../src/components/Glow";
import { api } from "../src/lib/api";

// Types for application data from backend
interface BackendJob {
  _id: string;
  title: string;
  designation?: string;
  description?: { details?: string; stipend?: { amount?: string | number } };
  company?: { name: string; logoUrl?: string; bannerImageUrl?: string };
  location?: string;
  labels?: string[];
  applicationDeadline?: Date;
  startDate?: Date;
  endDate?: Date;
}

interface BackendApplication {
  _id: string;
  jobId: BackendJob;
  userId: any;
  applicantName: string;
  resumeUrl: string;
  portfolioUrl?: string;
  text: string;
  status: "PENDING" | "APPROVED" | "REJECTED" | "IN_PROGRESS";
  createdAt: string;
}

// Map backend status to UI status
const mapBackendStatusToUI = (
  status: string,
): "accepted" | "review" | "rejected" => {
  switch (status) {
    case "APPROVED":
      return "accepted";
    case "REJECTED":
      return "rejected";
    case "IN_PROGRESS":
      return "review";
    case "PENDING":
    default:
      return "review";
  }
};

const statusColor: Record<string, string> = {
  accepted: colors.green,
  confirmed: colors.green,
  review: colors.amber,
  rejected: colors.red,
};
const statusIcon: Record<string, any> = {
  accepted: "checkmark",
  confirmed: "checkmark",
  review: "time-outline",
  rejected: "close",
};
const statusLabel: Record<string, string> = {
  accepted: "Accepted",
  confirmed: "Confirmed",
  review: "In review",
  rejected: "Not selected",
};

export default function Tracker() {
  const insets = useSafeAreaInsets();
  const router = useRouter();
  const { user, loading: authLoading } = useAuth();
  const [menu, setMenu] = useState(false);
  const [sb, setSb] = useState<{ width: number; height: number } | null>(null);
  const [apps, setApps] = useState<BackendApplication[]>([]);
  const [loading, setLoading] = useState(true);
  const [openIdx, setOpenIdx] = useState<number | null>(null);
  const detail = openIdx != null ? apps[openIdx] : null;

  useEffect(() => {
    if (!authLoading && !user) {
      router.replace("/");
    }
  }, [user, authLoading, router]);

  if (authLoading) {
    return (
      <View
        style={{
          flex: 1,
          backgroundColor: colors.bg,
          alignItems: "center",
          justifyContent: "center",
        }}
      >
        <ActivityIndicator color="#fff" />
      </View>
    );
  }

  // Helper to clean up URLs with extra quotes/backticks
  const cleanUrl = (url?: string) => {
    if (!url) return undefined;
    return url.replace(/^[\s`'"]+|[\s`'"]+$/g, "").trim();
  };

  // Fetch applications
  const fetchApplications = async () => {
    setLoading(true);
    try {
      const result = await api.applications.getMyApplications({ offset: 50 });
      if (result.success && result.data) {
        setApps(result.data);
      }
    } catch (error: any) {
      console.error("Error fetching applications:", error);
      Alert.alert("Error", error.message || "Failed to load applications");
    } finally {
      setLoading(false);
    }
  };

  useEffect(() => {
    fetchApplications();
  }, []);

  const formatDate = (dateStr: string) => {
    const date = new Date(dateStr);
    return `Applied ${date.toLocaleDateString("en-US", { month: "short", day: "numeric" })}`;
  };

  const getJobTitle = (app: BackendApplication) => {
    return app.jobId?.title || "Opportunity";
  };

  const getJobOrg = (app: BackendApplication) => {
    const parts = [];
    if (app.jobId?.company?.name) parts.push(app.jobId.company.name);
    if (app.jobId?.location) parts.push(app.jobId.location);
    return parts.join(" · ");
  };

  const getJobLetter = (app: BackendApplication) => {
    return getJobTitle(app)[0]?.toUpperCase() || "?";
  };

  return (
    <View style={[styles.root, { paddingTop: insets.top }]}>
      <View style={styles.header}>
        <Pressable style={styles.iconBtn} onPress={() => setMenu(true)}>
          <Ionicons name="menu" size={20} color="#fff" />
        </Pressable>
        <Text style={styles.htitle}>Tracker</Text>
        <View style={{ flex: 1 }} />
        <Pressable style={styles.iconBtn}>
          <Ionicons name="notifications-outline" size={19} color="#fff" />
        </Pressable>
      </View>

      {loading ? (
        <View
          style={{ flex: 1, justifyContent: "center", alignItems: "center" }}
        >
          <ActivityIndicator color="#fff" />
          <Text style={[styles.empty, { marginTop: 12 }]}>
            Loading applications...
          </Text>
        </View>
      ) : (
        <ScrollView contentContainerStyle={{ padding: 16, paddingTop: 6 }}>
          {apps.map((a, i) => (
            <Pressable
              key={a._id}
              style={styles.card}
              onPress={() => setOpenIdx(i)}
            >
              {cleanUrl(a.jobId?.company?.bannerImageUrl) ? (
                <Image
                  source={{ uri: cleanUrl(a.jobId?.company?.bannerImageUrl)! }}
                  style={styles.thumb}
                />
              ) : cleanUrl(a.jobId?.company?.logoUrl) ? (
                <Image
                  source={{ uri: cleanUrl(a.jobId?.company?.logoUrl)! }}
                  style={styles.thumb}
                />
              ) : (
                <View style={styles.letter}>
                  <Text style={styles.letterText}>{getJobLetter(a)}</Text>
                </View>
              )}
              <View style={{ flex: 1 }}>
                <Text style={styles.title} numberOfLines={1}>
                  {getJobTitle(a)}
                </Text>
                <Text style={styles.sub} numberOfLines={1}>
                  {getJobOrg(a).split(" · ")[0]} · {formatDate(a.createdAt)}
                </Text>
              </View>
              <View
                style={[
                  styles.ring,
                  {
                    backgroundColor:
                      statusColor[mapBackendStatusToUI(a.status)],
                  },
                ]}
              >
                <Ionicons
                  name={statusIcon[mapBackendStatusToUI(a.status)]}
                  size={11}
                  color="#fff"
                />
              </View>
            </Pressable>
          ))}
          {apps.length === 0 && (
            <Text style={styles.empty}>No applications yet.</Text>
          )}
        </ScrollView>
      )}

      <Portal
        visible={detail != null}
        animationType="slide"
        onRequestClose={() => setOpenIdx(null)}
      >
        <View style={[styles.detail, { paddingTop: insets.top }]}>
          <LinearGradient
            pointerEvents="none"
            colors={[
              detail
                ? statusColor[mapBackendStatusToUI(detail.status)] + "14"
                : "transparent",
              "transparent",
            ]}
            style={{
              position: "absolute",
              top: 0,
              left: 0,
              right: 0,
              height: 200,
            }}
          />
          <View style={styles.dTop}>
            <Pressable style={styles.iconBtn} onPress={() => setOpenIdx(null)}>
              <Ionicons name="chevron-back" size={20} color="#fff" />
            </Pressable>
            <Text style={styles.dTopTitle}>Application</Text>
          </View>
          {detail && (
            <ScrollView
              contentContainerStyle={{ padding: 22, paddingBottom: 30 }}
            >
              <View
                style={{ flexDirection: "row", gap: 13, alignItems: "center" }}
              >
                {cleanUrl(detail.jobId?.company?.bannerImageUrl) ? (
                  <Image
                    source={{
                      uri: cleanUrl(detail.jobId?.company?.bannerImageUrl)!,
                    }}
                    style={styles.dIco}
                  />
                ) : cleanUrl(detail.jobId?.company?.logoUrl) ? (
                  <Image
                    source={{ uri: cleanUrl(detail.jobId?.company?.logoUrl)! }}
                    style={styles.dIco}
                  />
                ) : (
                  <View style={[styles.dIco, styles.letter]}>
                    <Text style={styles.letterText}>
                      {getJobLetter(detail)}
                    </Text>
                  </View>
                )}
                <View style={{ flex: 1 }}>
                  <Text style={styles.dTitle}>{getJobTitle(detail)}</Text>
                  <Text style={styles.sub}>{getJobOrg(detail)}</Text>
                </View>
              </View>
              <View style={styles.dTags}>
                {detail.jobId?.labels?.map((t) => (
                  <View key={t} style={styles.miniChip}>
                    <Text style={styles.miniChipText}>{t}</Text>
                  </View>
                ))}
              </View>
              <View style={{ flexDirection: "row", gap: 12, marginTop: 18 }}>
                <View style={{ flex: 1 }}>
                  <Text style={styles.dlabel}>APPLIED</Text>
                  <Text style={styles.dval}>
                    {formatDate(detail.createdAt).replace("Applied ", "")}
                  </Text>
                </View>
                <View style={{ flex: 1 }}>
                  <Text style={styles.dlabel}>JOINING</Text>
                  <Text style={styles.dval}>
                    {detail.jobId?.startDate
                      ? new Date(detail.jobId.startDate).toLocaleDateString(
                          "en-US",
                          { month: "short", day: "numeric", year: "numeric" },
                        )
                      : "—"}
                  </Text>
                </View>
              </View>
              <View
                style={[
                  styles.pill,
                  {
                    backgroundColor:
                      statusColor[mapBackendStatusToUI(detail.status)] + "22",
                    borderColor:
                      statusColor[mapBackendStatusToUI(detail.status)] + "66",
                    marginTop: 16,
                  },
                ]}
              >
                <View
                  style={[
                    styles.pillDot,
                    {
                      backgroundColor:
                        statusColor[mapBackendStatusToUI(detail.status)],
                    },
                  ]}
                />
                <Text
                  style={[
                    styles.pillText,
                    {
                      color:
                        mapBackendStatusToUI(detail.status) === "accepted"
                          ? colors.greenSoft
                          : mapBackendStatusToUI(detail.status) === "review"
                            ? colors.amberSoft
                            : colors.redSoft,
                    },
                  ]}
                >
                  {statusLabel[mapBackendStatusToUI(detail.status)]}
                </Text>
              </View>
              <Text style={styles.ih}>DESCRIPTION</Text>
              <Text style={styles.ibody}>
                {detail.jobId?.description?.details ||
                  "No description available"}
              </Text>
              <Text style={styles.ih}>STIPEND</Text>
              <Text style={styles.dPay}>
                {detail.jobId?.description?.stipend?.amount
                  ? `${detail.jobId.description.stipend.amount}/month`
                  : "—"}
              </Text>
            </ScrollView>
          )}
        </View>
      </Portal>

      <Menu visible={menu} onClose={() => setMenu(false)} current="/tracker" />
    </View>
  );
}

const styles = StyleSheet.create({
  root: { flex: 1, backgroundColor: colors.bg },
  header: {
    flexDirection: "row",
    alignItems: "center",
    gap: 10,
    paddingHorizontal: 20,
    paddingTop: 14,
    paddingBottom: 8,
  },
  iconBtn: {
    width: 38,
    height: 38,
    borderRadius: 12,
    alignItems: "center",
    justifyContent: "center",
    backgroundColor: colors.surface,
    borderWidth: 1,
    borderColor: colors.hairline,
  },
  htitle: {
    color: "#fff",
    fontFamily: font.semibold,
    fontSize: 19,
    letterSpacing: -0.3,
  },
  card: {
    flexDirection: "row",
    alignItems: "center",
    gap: 10,
    paddingVertical: 8,
    paddingHorizontal: 11,
    borderRadius: 18,
    marginBottom: 9,
    backgroundColor: colors.surface,
    borderWidth: 1,
    borderColor: colors.hairline,
  },
  thumb: { width: 40, height: 40, borderRadius: 12 },
  letter: {
    width: 40,
    height: 40,
    borderRadius: 12,
    backgroundColor: "#3a3a42",
    alignItems: "center",
    justifyContent: "center",
  },
  letterText: { color: "#fff", fontFamily: font.semibold, fontSize: 15 },
  title: { color: "#fff", fontFamily: font.semibold, fontSize: 14 },
  sub: {
    color: colors.textDim,
    fontFamily: font.regular,
    fontSize: 11.5,
    marginTop: 2,
  },
  ring: {
    width: 22,
    height: 22,
    borderRadius: 11,
    alignItems: "center",
    justifyContent: "center",
  },
  empty: {
    color: colors.textFaint,
    fontFamily: font.regular,
    fontSize: 13,
    textAlign: "center",
    paddingTop: 40,
  },
  detail: { flex: 1, backgroundColor: colors.bg },
  dTop: {
    flexDirection: "row",
    alignItems: "center",
    gap: 10,
    paddingHorizontal: 16,
    paddingVertical: 6,
  },
  dTopTitle: { color: "#fff", fontFamily: font.semibold, fontSize: 15 },
  dIco: { width: 56, height: 56, borderRadius: 16 },
  dTitle: {
    color: "#fff",
    fontFamily: font.bold,
    fontSize: 20,
    letterSpacing: -0.4,
  },
  dTags: { flexDirection: "row", flexWrap: "wrap", gap: 7, marginTop: 14 },
  miniChip: {
    paddingVertical: 5,
    paddingHorizontal: 11,
    borderRadius: 13,
    backgroundColor: "rgba(255,255,255,0.1)",
    borderWidth: 1,
    borderColor: "rgba(255,255,255,0.16)",
  },
  miniChipText: { color: "#fff", fontFamily: font.medium, fontSize: 11 },
  dlabel: {
    color: colors.textFaint,
    fontFamily: font.semibold,
    fontSize: 10.5,
    letterSpacing: 0.3,
  },
  dval: {
    color: "#fff",
    fontFamily: font.semibold,
    fontSize: 14,
    marginTop: 4,
  },
  pill: {
    flexDirection: "row",
    alignItems: "center",
    gap: 6,
    alignSelf: "flex-start",
    paddingVertical: 5,
    paddingHorizontal: 11,
    borderRadius: 13,
    borderWidth: 1,
  },
  pillDot: { width: 7, height: 7, borderRadius: 4 },
  pillText: { fontFamily: font.semibold, fontSize: 11.5 },
  ih: {
    color: colors.textDim,
    fontFamily: font.semibold,
    fontSize: 12,
    letterSpacing: 0.4,
    marginTop: 22,
    marginBottom: 8,
  },
  ibody: {
    color: "rgba(255,255,255,0.72)",
    fontFamily: font.light,
    fontSize: 13,
    lineHeight: 21,
  },
  req: {
    color: "rgba(255,255,255,0.72)",
    fontFamily: font.light,
    fontSize: 12.5,
    flex: 1,
    lineHeight: 19,
  },
  dPay: { color: "#fff", fontFamily: font.semibold, fontSize: 16 },
  confirm: {
    height: 54,
    borderRadius: radius.pill,
    alignItems: "center",
    justifyContent: "center",
    backgroundColor: colors.green,
  },
  confirmText: { color: "#06210f", fontFamily: font.bold, fontSize: 15 },
});
