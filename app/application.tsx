...
export default function Application() {
  ...
  // Pre-fill with existing resume
  useEffect(() => {
    if (userProfile?.resumeUrl) {
      // Extract filename from URL or use a default
      const filename =
        userProfile.resumeUrl.split("/").pop() || "Uploaded Resume";
      setCv(filename);
    }
  ...
  const submit = () => {
    // Check if CV is uploaded first
    if (!cv && !userProfile?.resumeUrl) {
      Alert.alert(
        "Resume Required",
        "Please upload your resume to submit an application!",
      );
      return;
    }
    setSent(true);
  ...
  return (
    <View style={[styles.root, { paddingTop: insets.top }]}>
      ...
      <ScrollView contentContainerStyle={{ padding: 18 }}>
        ...
        <Pressable
          style={[styles.upload, cv && styles.uploadHas]}
          onPress={pickCv}
        >
      ...
      <View style={{ padding: 18, paddingBottom: insets.bottom + 16 }}>
        <Pressable
          style={[styles.submit, sent && styles.submitDone]}
          onPress={submit}
        >
          <Text
            style={[styles.submitText, sent && { color: colors.greenSoft }]}
          >
